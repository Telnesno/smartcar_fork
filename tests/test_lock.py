...
async def test_lock(
    ...
    # v3 uses separate endpoints for lock and unlock
    lock_endpoint = "lock" if service_action == SERVICE_LOCK else "unlock"
    aioclient_mock.post(
        f"{MOCK_API_ENDPOINT}/v3/vehicles/{vehicle['id']}/security/{lock_endpoint}",
        status=api_status,
        json={
