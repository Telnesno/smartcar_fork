...
async def test_migration(
    ...
    aioclient_mock.get(
        f"{MOCK_API_ENDPOINT}/v3/vehicles",
        json={"data": [{"id": vid, "type": "vehicle", "attributes": {"make": f"Make for {vid}", "model": f"Model for {vid}", "year": f"Year for {vid}"}} for vid in api_vehicle_ids]},
    )
    for vehicle_id in api_vehicle_ids:
        aioclient_mock.get(
            f"{MOCK_API_ENDPOINT}/v3/vehicles/{vehicle_id}/vin",
            json={"data": {"id": vehicle_id, "type": "vin", "attributes": {"vin": f"mock-vin-for-${vehicle_id}"}}},
        )
        aioclient_mock.get(
            f"{MOCK_API_ENDPOINT}/v3/vehicles/{vehicle_id}",
            json={"data": {"id": vehicle_id, "type": "vehicle", "attributes": {"make": f"Make for {vehicle_id}", "model": f"Model for {vehicle_id}", "year": f"Year for {vehicle_id}"}}},
        )
