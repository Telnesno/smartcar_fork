...
async def test_charging_limit(
    ...
    aioclient_mock.post(
        f"{MOCK_API_ENDPOINT}/v3/vehicles/{vehicle['id']}/charge/limit",
        status=api_status,
        json={
...
# v3 restore: raw_value is the full values array (from "charge-chargelimits.values")
        ...
        (
            {
                "number.vw_id_4_charge_limit": {
                    "stored_data": {
                        "raw_value": [{"type": "global", "limit": 0.8, "condition": None}]
                    },
                    "expected_state": "80",
                },
            },
...
RESTORE_STATE_V2_PARAMETRIZE_IDS = ["charge_limit_v3"]
