...
async def test_polling_v3_sensor(
    ...
    """Test sensor refresh works for all sensors via v3 GET /signals."""
    ...
    # In v3, all signals are fetched via GET /signals for any entity update
    await async_update_entity(hass, "sensor.vw_id_4_low_voltage_battery")

    # Should have made 1 call to GET /signals
    assert aioclient_mock.call_count == 1
    assert "Unsupported update requests" not in caplog.text
...
async def test_polling_v3_sensor_retains_healthy_coordinator(
    ...
    await async_update_entity(hass, "sensor.vw_id_4_low_voltage_battery")
    # v3 fetches all signals; battery should remain available
    assert hass.states.get("sensor.vw_id_4_battery").state != "unavailable"
    assert "Unsupported update requests" not in caplog.text
...
# v3 restore state: raw_value for tire pressure is the full values array
RESTORE_STATE_V2_PARAMETRIZE_ARGS = [
    ...
    [
        (
            {
                # In v3, raw_value is the full array from "wheel-tires.values"
                "sensor.vw_id_4_tire_pressure_front_left": {
                    "stored_data": {
                        "raw_value": [
                            {"column": VEHICLE_LEFT_COLUMN, "row": VEHICLE_FRONT_ROW, "tirePressure": 235},
                            {"column": VEHICLE_LEFT_COLUMN, "row": VEHICLE_BACK_ROW, "tirePressure": 234},
                            {"column": VEHICLE_RIGHT_COLUMN, "row": VEHICLE_FRONT_ROW, "tirePressure": 233},
                            {"column": VEHICLE_RIGHT_COLUMN, "row": VEHICLE_BACK_ROW, "tirePressure": 232},
                        ]
                    },
                    "expected_state": "235",
                },
...
RESTORE_STATE_V2_PARAMETRIZE_IDS = ["tire_pressures_v3"]
