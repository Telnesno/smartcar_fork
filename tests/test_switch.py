...
async def test_switch(
    ...
    # v3 uses separate endpoints for start and stop
    charge_endpoint = "start" if service_action == SERVICE_TURN_ON else "stop"
    aioclient_mock.post(
        f"{MOCK_API_ENDPOINT}/v3/vehicles/{vehicle['id']}/charge/{charge_endpoint}",
        status=api_status,
        json={
...
# v3 restore: raw_value is a bool (from "charge-ischarging.value")
...
RESTORE_STATE_V2_PARAMETRIZE_IDS = ["is_charging_v3"]
