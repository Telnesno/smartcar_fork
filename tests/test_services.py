...
async def test_door_closure(
    ...
    # v3 uses separate lock and unlock endpoints
    security_action = "unlock" if "unlock" in call else "lock"
    aioclient_mock.post(
        f"{MOCK_API_ENDPOINT}/v3/vehicles/{vehicle['id']}/security/{security_action}",
        status=api_status,
        json={
