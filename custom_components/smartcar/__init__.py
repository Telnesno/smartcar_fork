...
async def _store_all_vehicles(
    ...
    try:
        ...
        # v3: {"data": [{"id": "...", "type": "vehicle", ...}]}
        # v2: {"vehicles": ["id1", "id2"]}
        if "data" in vehicle_list_data:
            vehicle_ids = [v["id"] for v in vehicle_list_data["data"] if "id" in v]
        else:
...
async def _store_vehicle_details(
    ...
    try:
        ...
        # v3: {"data": {"attributes": {"vin": "..."}}}
        # v2: {"vin": "..."}
        vin = (
            vin_data.get("data", {}).get("attributes", {}).get("vin")
            or vin_data.get("vin")
        )
        ...
        # v3: {"data": {"attributes": {"make": ..., "model": ..., "year": ...}}}
        # v2: {"make": ..., "model": ..., "year": ...}
        attrs = vehicle_info.get("data", {}).get("attributes") or vehicle_info
        make = attrs.get("make")
